...
import {
  Calendar, Check, ArrowRight, Loader2, AlertCircle,
  PawPrint, ChevronRight, ChevronLeft, Phone,
} from "lucide-react";
...
function validateIranianPhone(phone: string): boolean {
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="clay-card p-6 mb-8">
          ...
          <div className="grid grid-cols-7 gap-1">
            {calendarDays.map((day, i) => {
              ...
              return <button key={`d-${i}`} onClick={() => !past && handleDateClick(day)} disabled={past} className={`aspect-square rounded-xl flex flex-col items-center justify-center text-sm font-bold transition-all ${past ? "text-muted-foreground/30 cursor-not-allowed bg-muted/30" : selected ? "bg-primary text-primary-foreground shadow-md scale-105" : "hover:bg-secondary/60 hover:scale-105"}`}>{toPersianDigits(day)}{past && viewYear === todayJ[0] && viewMonth === todayJ[1] && day === todayJ[2] && <span className="text-[8px] text-muted-foreground/40 mt-0.5">امروز</span>}</button>;
            })}
          </div>
        </motion.div>
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="clay-card p-6 mb-8 overflow-hidden">
            ...
            <div className="grid grid-cols-4 sm:grid-cols-8 gap-2">
              {["09:00","09:30","10:00","10:30","11:00","11:30","12:00","12:30","14:00","14:30","15:00","15:30","16:00","16:30","17:00","17:30"].map((time) => (
                <button key={time} onClick={() => setSelectedTime(time)} className={`py-2.5 px-3 rounded-xl text-sm font-bold transition-all ${selectedTime === time ? "bg-primary text-primary-foreground shadow-md" : "clay-card hover:bg-secondary/50"}`}>{toPersianDigits(time)}</button>
              ))}
            </div>
          </motion.div>
        )}</AnimatePresence>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="clay-card p-6 mb-8">
            ...
            <div className="space-y-5">
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">{services.map((s) => <button key={s._id} onClick={() => setSelectedServiceId(s._id)} className={`clay-card p-3 text-right transition-all ${selectedServiceId === s._id ? "ring-2 ring-primary" : "hover:bg-secondary/50"}`}><div className="font-bold text-sm">{s.name}</div><div className="text-sm font-black text-primary mt-1">{toPersianDigits(s.price.toLocaleString())} تومان</div></button>)}</div>
              </div>
              <div>
                <label className="block text-sm font-bold mb-2">نوع حیوان *</label>
                <div className="flex gap-2">{[["dog","🐕 سگ"],["cat","🐈 گربه"],["rabbit","🐇 خرگوش"]].map(([v, l]) => <button key={v} onClick={() => setPetType(v)} className={`clay-card flex-1 py-3 text-center text-sm font-bold transition-all ${petType === v ? "ring-2 ring-primary" : "hover:bg-secondary/50"}`}>{l}</button>)}</div>
              </div>
              ...
              <div>
                ...
                <div className="relative"><Phone className="absolute right-3 top-3 h-4 w-4 text-muted-foreground" /><input type="tel" value={phone} onChange={(e) => handlePhoneChange(e.target.value)} placeholder="09121234567" dir="ltr" maxLength={11} className={`clay-input w-full pr-9 pl-4 py-3 text-sm border focus:outline-none focus:ring-2 focus:ring-primary ${phoneError ? "border-red-400" : ""}`} /></div>
                {phoneError && <p className="text-red-500 text-xs mt-1">{phoneError}</p>}
                {!phoneError && phone.length === 11 && validateIranianPhone(phone) && <p className="text-emerald-600 text-xs mt-1 flex items-center gap-1"><Check className="w-3 h-3" /> شماره معتبر است</p>}
              </div>
