...
export default function AdminLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (username.length < 3) {
      setError("نام کاربری حداقل ۳ کاراکتر باشد");
      return;
    }
    if (password.length < 4) {
      setError("رمز عبور حداقل ۴ کاراکتر باشد");
      return;
    }
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-4" dir="rtl">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="clay-card p-6 space-y-4">
          ...
          <div><label className="block text-sm font-bold mb-1.5">نام کاربری</label><div className="relative"><User className="absolute right-3 top-3 h-4 w-4 text-muted-foreground" /><input type="text" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="نام کاربری" className="clay-input w-full pr-9 pl-4 py-2.5 text-sm border border-border focus:outline-none focus:ring-2 focus:ring-primary" required autoFocus autoComplete="username" /></div></div>
          <div><label className="block text-sm font-bold mb-1.5">رمز عبور</label><div className="relative"><Lock className="absolute right-3 top-3 h-4 w-4 text-muted-foreground" /><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="رمز عبور" className="clay-input w-full pr-9 pl-4 py-2.5 text-sm border border-border focus:outline-none focus:ring-2 focus:ring-primary" required autoComplete="current-password" /></div></div>
